...
class CSVDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = data.values.astype('float32')
        
        if self.transform:
            sample = self.transform(sample)
        
        return sample, sample  # Assuming you want to return the sample as both input and target.
...
def main(args):
    ...
    dataset_train = CSVDataset(os.path.join(args.data_path, 'train'), transform=transform_train)
    print(dataset_train)
