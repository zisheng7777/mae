...
class MaskedAutoencoderViT(nn.Module):
    ...
    """
    def __init__(self, img_size=4, patch_size=16, in_chans=1,
...
#################################################################################
    def unpatchify(self, x):
        """
        Reshape the transformed model output back to image space.
        Assumes the input shape is [N, L, D], where L is the number of patches and D is the patch dimension.
        """
        p = self.patch_embed.patch_size[0]  # Side length of each patch
        h = w = int((x.shape[1] ** 0.5))  # Assume the image and patches are square-shaped

        # Reshape x to [N, h, w, p, p], considering we have only one channel
        x = x.reshape(x.shape[0], h, w, p, p)

        # Reshape and reorder to reconstruct the original image dimensions
        x = x.permute(0, 3, 1, 4, 2)  # Rearrange to [N, p, h, p, w]
        x = x.reshape(x.shape[0], p * h, p * w)  # Reshape to [N, H, W]
...
#################################################################################
    ...
    def forward_encoder(self, x, mask_ratio, custom_mask=None):
        ...
        x, mask, ids_restore = self.random_masking(x, mask_ratio, custom_mask)
...
def mae_vit_base_patch56_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=56, embed_dim=768, depth=12, num_heads=12,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
...
mae_vit_base_patch56 = mae_vit_base_patch56_dec512d8b
mae_vit_base_patch1 = mae_vit_base_patch1_dec512d8b
